...
from scripts.step4_table_utils_ali_multi_thread import generate_table_description as generate_table_description_ali  # 原有阿里模型
from scripts.step4_table_utils_multi_thresd import generate_table_description as generate_table_description_fallback  # 新增fallback模型
from scripts.step4_compress_image import compress_image
from scripts.step4_describe_not_table_images_PaddleOCR import run_paddle_ocr_on_image  # 最终兜底

from file_paths_config import OUTPUT_DIR
from common_utils import safe_json_dump

# 获取脚本所在目录，确保日志文件生成在该目录下
script_dir = os.path.dirname(os.path.abspath(__file__))
log_file = os.path.join(script_dir, "table_detection.log")
...
def process_pdf_table_descriptions(pdf_info, max_workers=10):
    ...
    describe_json_path = os.path.join(pdf_folder_path, f"{pdf_folder_name}_table_describe.json")
