...
# ======= LLM生成描述、压缩图片 =======
from scripts.step4_table_utils_multi_thresd import generate_table_description  # 调用生成描述的函数
from scripts.step4_compress_image import compress_image  # 调用压缩图片函数

# ======= 引入PaddleOCR提供的函数 =======
from scripts.step4_describe_not_table_images_PaddleOCR import run_paddle_ocr_on_image
...
log_file = os.path.join(script_dir, "not_table_detection.log")
...
def process_pdf_not_table_descriptions(pdf_info, lock):
    ...
    json_output_path = os.path.join(pdf_folder_path, f"{pdf_folder_name}.json")
    ...
    for idx, img_file in enumerate(img_files):
        ...
        try:
            ...
            description = generate_table_description(image_path)  # 第一次LLM尝试
